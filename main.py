...
from google_docs_publisher import publish_to_google_drive
...
def main():
    ...
    if generated_contents and not args.dry_run:
        doc_url = publish_to_google_drive(generated_contents, today_str)
        if doc_url:
            print(f"\n{'='*60}")
            print(f"  완료! Google Docs 문서가 생성되었습니다.")
            print(f"  {doc_url}")
            print(f"{'='*60}\n")
    elif args.dry_run:
        print(f"\n[드라이런] {len(generated_contents)}건 처리 완료 (Drive 업로드 생략)")
